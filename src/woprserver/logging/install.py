...
from .neutralizers import disarm_uvicorn, neutralize_mlflow, neutralize_mlserver
from .record import install_logrecord_factory
from .routing import install_global_handler_router, install_hard_router, lock_root
...
def configure_logger(settings: Optional[object] = None) -> logging.Logger:
    ...
    try:
        _maybe_apply_logging_file(
            getattr(settings, "logging_settings", None) if settings else None
        )
    except Exception:
        pass
    return logger
    ...
    if isinstance(logging_settings, str) and Path(logging_settings).is_file():
        ...
        if path.suffix.lower() == ".json":
            ...
            logging.config.fileConfig(
                fname=str(path), disable_existing_loggers=False
            )
